...
import topdf
from topdf import cli, converters, pipeline
...
def folder(tmp_path):
    ...
    for name in ("analysis.ipynb", "analysis.json", "notes.md", "fit.py", "run.log"):
        write(root / name)
    write(root / "geometry.xyz", "3\nwater\nO 0 0 0.117\n")  # text, no highlighter for .xyz
    (root / "spectrum.png").write_bytes(PNG_HEAD)
    (root / "analysis.pdf").write_bytes(FAKE_PDF)  # an earlier run's output
    write(root / ".ipynb_checkpoints" / "analysis-checkpoint.ipynb")
    write(root / "raw" / "extra.json")
    return root
...
def test_folder_contributes_the_files_that_print_in_name_order(folder):
    ...
    assert names(files) == ["analysis.ipynb", "analysis.json", "fit.py", "geometry.xyz", "notes.md", "run.log"]
    assert names(skipped) == ["analysis.pdf", "spectrum.png"]
    assert missing == []


def test_recursive_folder_skips_hidden_folders(folder):
    files, _, _ = cli.collect([str(folder)], recursive=True)
    assert "extra.json" in names(files)
    assert "analysis-checkpoint.ipynb" not in names(files)
...
def test_wildcards_expand_and_misses_are_reported(folder):
    ...
    assert names(files) == ["analysis.json"]
...
def test_converts_into_the_default_folder(isolated_pipeline, stub_engine, folder, capsys):
    assert cli.main([str(folder / "analysis.ipynb"), str(folder / "analysis.json")]) == 0
    out = isolated_pipeline["DEFAULT_OUTPUT_DIR"]
    assert sorted(p.name for p in out.iterdir()) == ["analysis.json.pdf", "analysis.pdf"]
    lines = capsys.readouterr().out.splitlines()
    assert lines[0] == f"[1/2] analysis.ipynb: Done -> {out / 'analysis.pdf'}"
...
def test_skipped_folder_files_are_reported(isolated_pipeline, stub_engine, folder, capsys):
    ...
    assert "Skipped 2 files (PDF or not text): analysis.pdf, spectrum.png" in captured.err
...
def test_out_and_next_to_source(isolated_pipeline, stub_engine, folder, tmp_path):
    ...
    assert (folder / "raw" / "extra.json.pdf").exists()
...
def test_version(capsys):
    ...
    assert capsys.readouterr().out.strip() == f"topdf {topdf.__version__}"
